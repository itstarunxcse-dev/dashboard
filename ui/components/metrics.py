        ...
        st.markdown(f"""
            <div class="metric-box">
                ...
                <div class="metric-value" style="color: #ffd700; background: none; -webkit-text-fill-color: #ffd700;">
                    {metrics.market_annual_return:.2f}%
                </div>
            </div>
        """, unsafe_allow_html=True)
...
def render_trade_history(metrics: BacktestMetrics):
    ...
    total_wins = sum([t.profit_loss for t in metrics.trades if t.profit_loss > 0])
    total_losses = abs(sum([t.profit_loss for t in metrics.trades if t.profit_loss <= 0]))
    
    avg_win_price = total_wins / winning_trades if winning_trades > 0 else 0
    avg_loss_price = total_losses / losing_trades if losing_trades > 0 else 0
    ...
    with col3:
        st.markdown(f"""
            <div class="metric-box">
                ...
                <div class="metric-value" style="color: #00ff7f; background: none; -webkit-text-fill-color: #00ff7f;">INR {avg_win_price:.2f}</div>
            </div>
        """, unsafe_allow_html=True)
    with col4:
        st.markdown(f"""
            <div class="metric-box">
                ...
                <div class="metric-value" style="color: #ff6347; background: none; -webkit-text-fill-color: #ff6347;">INR {avg_loss_price:.2f}</div>
            </div>
        """, unsafe_allow_html=True)
    with col5:
        st.markdown(f"""
            <div class="metric-box">
                ...
                <div class="metric-value" style="color: #ffd700; background: none; -webkit-text-fill-color: #ffd700;">INR {profit_factor:.2f}</div>
            </div>
        """, unsafe_allow_html=True)
...
def render_confidence(metrics: BacktestMetrics):
    """
    ...
    confidence = min(metrics.confidence_ratio * 100, 100.0)
